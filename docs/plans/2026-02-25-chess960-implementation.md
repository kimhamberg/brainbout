...
export function applyUciMove(game: GameState, uciStr: string): GameState | null {
  const move = parseUci(uciStr);
  if (!move) return null;
...
import { createGame, makeMove, applyUciMove, getGameStatus, GameState } from './game';
...
function updateBoard(): void {
  api.set({
    ...
    movable: {
      ...
      dests: game.turn === playerColor ? game.dests as Map<string, string[]> : new Map(),
    },
  });
}
...
<div id="settings">
  ...
  <label>
    Difficulty (Elo)
    <input type="range" id="elo-slider" min="1320" max="3190" value="1500" step="10">
  ...
  <label>
    ...
    <input type="range" id="time-slider" min="200" max="5000" value="1000" step="100">
    <span id="time-value">1000</span>
  </label>
  <label>
    Position ID (0-959, blank = random)
    <input type="number" id="position-id" min="0" max="959" placeholder="random">
  </label>
  <label>
    Play as
