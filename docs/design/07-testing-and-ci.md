...
- **New — bundle-size gate (VH-6):** after `bun run build`, assert total JS ≤ **200 KB gzip** + a tree-shake assertion that WebGPU + unused filters drop out. Fail the build over budget (mirrors the atlas-digest gate).
